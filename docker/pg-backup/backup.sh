...
# PostgreSQL → Telegram backup cron for antispam-bot.
#
# Adapted for this project:
#   - DB service is `postgres` (not `db`).
#   - TELEGRAM_LOG_CHAT_ID is comma-separated (multiple super-admins);
#     the dump is uploaded to each one.
#   - GRASPIL_PROXY_URL optionally overrides the Telegram API root.
#
# Set as the container entrypoint: installs runtime deps, writes a crontab,
# runs one backup immediately, then starts crond in the foreground.
...
# TELEGRAM_LOG_CHAT_ID may be comma-separated (multiple super-admins).
# Split and upload to each chat id, skipping empties.
...
send_one() {
  ...
  if ! "$@" > /dev/null; then
    echo "[ERROR] Telegram upload failed for chat_id=${chat_id}" >&2
    return 1
  fi

  if ! grep -q '"ok":true' "${RESP_FILE}"; then
    echo "[ERROR] Telegram API returned non-ok response for chat_id=${chat_id}: $(cat "${RESP_FILE}")" >&2
    return 1
  fi

  echo "[INFO] Telegram send OK (chat_id=${chat_id})"
  return 0
}

# IFS-based split over commas.
IFS_OLD="${IFS:-}"
IFS=','
set -f
ok=0
fail=0
for id in ${TELEGRAM_LOG_CHAT_ID}; do
  id_trimmed="$(printf '%s' "$id" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
  if [ -z "$id_trimmed" ]; then
    continue
  fi
  if send_one "$id_trimmed"; then
    ok=$((ok + 1))
  else
    fail=$((fail + 1))
  fi
...
echo "[INFO] Backup ${FILENAME}: delivered to ${ok} chat(s)${fail:+, ${fail} failed}"
# Non-zero exit only if nothing was delivered.
[ "$ok" -gt 0 ]
EOF
